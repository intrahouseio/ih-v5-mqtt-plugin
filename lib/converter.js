    ...
    channels.forEach(item => {
      if (item.id && item.topic) {
        if (item.parentnodefolder) {
          ...
          this.subNodeMap.get(item.parenttopic).children.push(item.id);
        } else {
    ...
    if (this.subNodeMap.has(topic)) {
      ...
      if (scriptResult && Array.isArray(item.children)) {
        ...
        item.children.forEach(child => {
          if (scriptResult[child] != undefined) {
            if (typeof scriptResult[child] === "object") {
              data.push({ id: child, topic, value: scriptResult[child].value, ts:scriptResult[child].ts, chstatus:scriptResult[child].chstatus });
            } else {
              data.push({ id: child, topic, value: scriptResult[child], chstatus:0 });
            }            
          }
        });
      }
